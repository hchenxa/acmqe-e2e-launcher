...
function generate_spoke_context() {
    ...
    touch ${_spoke_context_path}/imported_kubeconfig

    # Generate the spoke cluster context
    if [[ ! -z $SPOKE_API_URL && ! -z $SPOKE_TOKEN ]]; then
        KUBECONFIG="${_spoke_context_path}/imported_kubeconfig" oc login --insecure-skip-tls-verify=true --token=$SPOKE_TOKEN $SPOKE_API_URL

    elif [[ ! -z $SPOKE_API_URL && ! -z $SPOKE_USERNAME && ! -z $SPOKE_PASSWORD ]]; then
        KUBECONFIG="${_spoke_context_path}/imported_kubeconfig" oc login --insecure-skip-tls-verify=true -u $SPOKE_USERNAME -p $SPOKE_PASSWORD $SPOKE_API_URL
    else
        ...
        if [[ $_spoke_cluster == "" ]]; then
            ...
            cp ${_spoke_context_path}/kubeconfig ${_spoke_context_path}/imported_kubeconfig
        else
