    ...
    if [[ ! -z $SPOKE_API_URL && ! -z $SPOKE_TOKEN ]]; then
        ...
        if [[ $_spoke_cluster == "" ]]; then
            ...
            if [[ $(uname -s) == "Darwin" ]]; then
                KUBECONFIG=${_spoke_context_path}/kubeconfig oc get secret -n $_spoke_cluster $secret_name --template={{.data.kubeconfig}} | base64 -D > ${_spoke_context_path}/imported-kubeconfig
            else
                KUBECONFIG=${_spoke_context_path}/kubeconfig oc get secret -n $_spoke_cluster $secret_name --template={{.data.kubeconfig}} | base64 -d > ${_spoke_context_path}/imported-kubeconfig
            fi
